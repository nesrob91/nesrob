...
"where id_manh = :idManh ";
    ...
    public boolean isCajaExistente(int idManh, String idLpn){
        ...
        try {
            ...
            return scpp.queryForObject(sql, parametros, Integer.class) > 0;
        } catch (Exception ex) {
            return false;
        }
    }
    ...
    public int getIdCajaByLpn(int idManh, String idLpn){
        ...
        try {
            ...
            return scpp.queryForObject(sql, parametros, Integer.class);
        } catch (Exception ex) {
            return 0;
        }
    }
    ...
    public DimensionesQB getDimensionesLpnEvaluado(int idManh, String idLpn){
        ...
        try {
            ...
            return scpp.queryForObject(sql, parametros, new DimensionesQBMapper());
        } catch (Exception ex) {
...
//", ICB.UNIT_HEIGHT "
    ...
    public List<Map<String,Object>> getPedidoMKP(List<String> adn, int idVendor, int idTipoPedido) {
        ...
        try {
            ...
            return scpp.queryForList(sql, parametros);
        } catch (Exception ex) {
    ...
    public List<Regla> getReglas(int idCanal, int tipoPedido) {
        ...
        String sql = " SELECT ID_CRITERIO "
                + " , TIPO "
                + " , PRIORIDAD "
                + " FROM INFO_CD.CE_CAT_CRITERIOS_COTIZACION "
                + " WHERE ID_TIPO = :idCanal "
                + " AND ID_TIPO_PEDIDO = :tipope"
                + " AND FLG_ACTIVO = 1 "
                + " ORDER BY PRIORIDAD";
        ...
        try {
            ...
            return scpp.query(sql, parametros, new ReglaMapper());
        } catch (Exception ex) {
    ...
    public FamiliaSap getFamiliaSap(String matkl) {
        ...
        try {
            ...
            return scpp.queryForObject(sql, parametros, new FamiliaSapMapper());
    ...
    public CarrierQB getMejorCarrierPostCalculo(int idManh, String idLpn) {
        ...
        try {
            ...
            return scpp.queryForObject(sql, parametros, new CarrierQBMapper());
        } catch (Exception ex) {
    ...
    public List<CarrierQB> getCarriersActivos(int idCanal, int tipoPedido) {
        ...
        try {
            ...
            return scpp.query(sql, parametros, new CarrierQBMapper());
        } catch (Exception ex) {
            System.out.println(ex.getMessage());
            return null;
        }
    ...
    public List<CarrierQB> getCarriersPrimeraIteracion(Paquete paquete) {
        ...
        String sql = " SELECT C.ID_CARRIER"
                + " , C.DESC_CARRIER"
                + " , C.COD_CARRIER "
                + " FROM INFO_CD.CE_CAT_CARRIERS C "
                + " , INFO_CD.ECOM_CB_HDR CH "
                + " WHERE C.ID_CARRIER = CH.ID_CARRIER "
                + " AND CH.PEDIDO = :pedido "
                + " AND CH.ID_CAJA = :idCaja "
                + " AND CH.CD_ID = :idManh "
                + " AND CH.ID_TIPO = :idCanal "
    ...
    public List<CarrierQB> getCarriersSiguienteIteracion(Paquete paquete, int idRegla) {
        ...
        String sql = " SELECT C.ID_CARRIER"
                ...
                + " , C.COD_CARRIER "
                + " FROM INFO_CD.CE_CAT_CARRIERS C "
                + " , INFO_CD.ECOM_CB_DTL CD "
                + " WHERE C.ID_CARRIER = CD.ID_CARRIER "
                + " AND CD.FLG_VALIDO = 1 "
                + " AND CD.PEDIDO = :pedido "
                + " AND CD.ID_CAJA = :idCaja "
                + " AND CD.CD_ID = :idManh "
                + " AND CD.ID_CRITERIO = :idRegla ";
    ...
    public boolean insertaCbHder2018(Paquete paquete, String usuario, CarrierQB carrierQB) {
        ...
        String sql = " INSERT INTO INFO_CD.ECOM_CB_HDR "
                + " ( "
                + " CD_ID, ID_CARRIER, PEDIDO, ID_CAJA, CP_DESTINO, FLG_PROCESADO, FLG_MEJOR_CARRIER, PESO_FISICO, PESO_VOLUMETRICO "
                + " , LARGO, ANCHO, ALTO, VOLUMEN, USUARIO_CREACION, FECHA_CREACION, USUARIO_MODIFICACION, FECHA_MODIFICACION "
                + " , REF_FIELD_1, REF_FIELD_2, REF_FIELD_3, PEDIDO_VTEX, ID_TIPO, ID_TIPO_PEDIDO, CP_ORIGEN, ID_LPN, COSTO "
                + " ) "
                + " VALUES "
                + " ( "
                + " :idManh, :idcarrier, :pedido, :idCaja, :cpDestino, :flgProcesado, :flgMejorCarrier, :pesoFisico, :pesoVolumetrico "
                + " , :largo, :ancho, :alto, :volumen, :usuarioCreacion, SYSDATE, NULL, NULL "
                + " , NULL, NULL, NULL, :pedidoVtex, :idCanal, :idTipoPedido, :cpOrigen, :idLpn, :costo "
                + " ) ";
        ...
        try {
            ...
            parametros.put("pesoFisico", paquete.getPesoFisico());
            parametros.put("pesoVolumetrico", paquete.getPesoVolumetrico());
            ...
            parametros.put("pedidoVtex", paquete.getPedidoAlterno());
            parametros.put("idCanal", paquete.getIdCanal());
            parametros.put("idTipoPedido", paquete.getIdTipoPedido());
            parametros.put("cpOrigen", paquete.getCpOrigen());
            parametros.put("idLpn", paquete.getIdLpn());
            parametros.put("costo", paquete.getCostoTotal());
    ...
    public boolean actualizaCbDtl(Paquete paquete, Regla regla, CarrierQB carrier, ResultadoEvaluacion resultadoEvaluacion) {
        ...
        String sql = " UPDATE INFO_CD.ECOM_CB_DTL A "
                + " SET A.FLG_PROCESADO = 1 "
                + " , A.USUARIO_MODIFICACION = :usuarioModificacion "
                + " , A.FECHA_MODIFICACION = SYSDATE "
                + (resultadoEvaluacion.isNulo() ? "" : " , CANTIDAD = :cantidad ")
                + " , A.TARIFA_BASE = :tarifaBase "
                + " , A.TARIFA_FINAL = :tarifaFinal "
                + " , REF_FIELD_1 = SUBSTR(:mensaje, 0, 45) "
                + " , FLG_VALIDO = :flgValido "
                + " WHERE PEDIDO = :pedido "
    ...
    public boolean actualizaCbHdr(Paquete paquete, CarrierQB carrier, String usuario, double tarifa) {
        ...
        String sql = " UPDATE INFO_CD.ECOM_CB_HDR "
                + " SET FLG_PROCESADO = 1 "
                + " , FLG_MEJOR_CARRIER = (CASE WHEN ID_CARRIER = :idCarrier THEN 1 ELSE 0 END) "
                + " , USUARIO_MODIFICACION = :usuarioModificacion "
                + " , FECHA_MODIFICACION = SYSDATE "
                + " , REF_FIELD_1 = (CASE WHEN ID_CARRIER = :idCarrier THEN :tarifa ELSE NULL END) "
                + " WHERE PEDIDO = :pedido "
                + " AND CD_ID = :idManh "
                + " AND ID_CAJA = :idCaja "
                + " AND ID_TIPO = :idCanal "
    ...
    public boolean insertaCbDtl(Paquete paquete, CarrierQB carrier, Regla regla, String usuario) {
        ...
        String sql = " INSERT INTO INFO_CD.ECOM_CB_DTL"
                + " (CD_ID, ID_CARRIER, PEDIDO, ID_CAJA, ID_CRITERIO, PRIORIDAD, TIPO, USUARIO_CREACION, FECHA_CREACION, FLG_PROCESADO, FLG_VALIDO) "
                + " VALUES "
                + " (:idManh, :idCarrier, :pedido, :idCaja, :idRegla, :prioridad, :tipo, :usuarioCreacion, SYSDATE, 0, 0) ";
        ...
        try {
            ...
            parametros.put("idCaja", paquete.getIdCaja());
            parametros.put("idRegla", regla.getIdRegla());
            parametros.put("prioridad", regla.getPrioridad());
            parametros.put("tipo", regla.getIdTipoRegla());
            parametros.put("usuarioCreacion", usuario);
    ...
    private int idCajaDtl(){
        ...
        try{
            return scpp.queryForObject(sql, new HashMap<String, Object>(), Integer.class) ;
        }catch (Exception ex) {
            System.out.println(ex.getMessage());
            return 0;
        }
    ...
    public int getIdContainer(double lenght, double width, double height, double weight, int idVendor, int idTipPed, boolean newFlg){
        ...
        if(newFlg)
            return scpp.queryForObject(newId, new HashMap<String, Object>(), Integer.class);
        else
            return scpp.queryForObject(getId, param, Integer.class);
    }
    ...
    private static final class CarrierQBMapper implements RowMapper<CarrierQB> {
        ...
        public CarrierQB mapRow(ResultSet rs, int i) throws SQLException {
            ...
            carrier.setDesdCarrier(rs.getString("DESC_CARRIER"));
            carrier.setCodCarrier(rs.getString("COD_CARRIER"));
            return carrier;
        }
