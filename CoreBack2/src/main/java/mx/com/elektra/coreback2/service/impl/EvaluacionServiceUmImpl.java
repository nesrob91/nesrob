...
public class EvaluacionServiceUmImpl implements EvaluacionService{
    ...
    public boolean actualizaCarriersEvaluados(Paquete paquete, Regla regla){
        ...
        if(idCarriersValidos.isEmpty()){
            return evaluacionDaoUm.actualizaCarriers(paquete, regla, idCarriers, 1, "actualizaCarriersEvaluados()");
        }
        else{
            idCarriersNoValidos = getIdCarriersNoValidos(idCarriers, idCarriersValidos);
            if(!idCarriersNoValidos.isEmpty()){
                evaluacionDaoUm.actualizaCarriers(paquete, regla, idCarriersNoValidos, 0, "actualizaCarriersEvaluados()");
            }
            return evaluacionDaoUm.actualizaCarriers(paquete, regla, idCarriersValidos, 1, "actualizaCarriersEvaluados()");
        }
